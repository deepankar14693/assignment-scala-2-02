...
def addBlogs(blog: Map[String,Int],add: Int,count: Int): String=
  {
   count match {
    case 0 => {val c = blog.maxBy(_._2)
      s"max blogs value are in $c"}
    case _ => {  if (add equals(0))
           {
      val add=total.nextInt((length - beg) + 1 )
      val updatedBlog = blog + ("scala" -> (blog("scala") + 1))
      addBlogs(updatedBlog,add,count-1)
              }
    else if (add equals(1))
    {
     val add=total.nextInt((length - beg) + 1 )
     val updatedBlog = blog + ("java" -> (blog("java") + 1))
     addBlogs(updatedBlog,add,count-1)
    }
     else
     {
      val add=total.nextInt((length - beg) + 1 )
      val updatedBlog = blog + ("kafka" -> (blog("kafka") + 1))
      addBlogs(updatedBlog,add,count-1)
    }
    }
   }
